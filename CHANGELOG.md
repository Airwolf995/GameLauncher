...
## [2.0.3] - 2026-06-22
### Geändert
- **Spielzeit-Konfiguration**: Gespeicherte Spielzeiten enthalten in `play_time` nun zusätzlich den lesbaren Spielnamen statt nur der ID, damit alte oder nicht mehr benötigte Einträge in der Config leichter zugeordnet und bereinigt werden können.

### Behoben
- **Spielzeit-Altbestände**: Bereits vorhandene `play_time`-Einträge im alten Format werden beim Laden weiterhin gelesen und beim App-Start automatisch auf das neue Format mit Spielname und Sekundenwert migriert.

## [2.0.2] - 2026-06-22
### Geändert
- **Einstellungen (Architektur)**: Das `SettingsViewModel` wurde entlastet. Autostart-Verwaltung, Dateidialoge, Update-Prüfung und GOG-Installationsstatus sind nun in eigenständige, testbare Dienste aufgeteilt.
- **Hauptfenster (Bild-Cache)**: Startup-Cache-Freigabe und Viewport-Retention der Spielecover wurden aus `MainWindow.xaml.cs` in einen eigenen Controller ausgelagert. Das Hauptfenster übernimmt damit nur noch die UI-Orchestrierung.
- **Visual-Tree-Hilfen**: Die zuvor mehrfach vorhandene rekursive Suche nach WPF-Kindelementen wurde in einer gemeinsamen Erweiterungsmethode zentralisiert.
- **Plattform-Scanner**: Steam-, Epic- und Xbox-Pfade werden über eine gemeinsame Pfadnormalisierung verarbeitet. EA und Ubisoft verwenden einen gemeinsamen Selektor zur Ermittlung plausibler ausführbarer Dateien.
- **Tests**: Die gemeinsamen Scanner-Hilfen wurden mit zusätzlichen Tests für Pfadnormalisierung, Deduplizierung und EXE-Auswahl abgesichert.
- **Bibliotheksansicht (Performance)**: Die Spielebibliothek nutzt nun einen stabileren Snapshot-/Zeilenaufbau mit freiem Pixel-Scrollen, konsolidierter Kartenlayout-Berechnung und gezielterem Refresh-Verhalten bei Suche, Sortierung und Filterwechsel.
- **App-Start (Ladepfad)**: Der Startscan wurde entzerrt. Langsamere Ubisoft-Erkennung wird nach dem ersten sichtbaren Bibliotheksaufbau nachgeladen, damit die App früher benutzbar wird.
- **Steam-Metadaten**: Steam-Beschreibungen, Genres, Entwickler und Veröffentlichungsdaten werden nun zusätzlich lokal zwischengespeichert und bei späteren Starts bevorzugt aus dem Cache übernommen.
- **Start-Cover-Preload**: Die Bildvorwärmung beim App-Start wurde auf einen einzigen kombinierten Preload-Pfad reduziert, damit weniger doppelte Cache- und Netzwerk-Arbeit entsteht.
- **Code-Hygiene (Bibliothek)**: Veraltete Filter-/Sortierpfade im `MainViewModel` sowie überholte Startup-Sonderlogik im Bildpfad wurden entfernt, um die Bibliothekslogik wieder übersichtlicher zu halten.

### Behoben
- **EA-Spielstart**: Erkannte EA-Spiele werden nicht mehr direkt über eine heuristisch ausgewählte EXE gestartet. Die EXE-Suche dient nur noch zur Icon-Ermittlung; den eigentlichen Start übernimmt die EA App über die maskierte Offer-ID im `origin2`-Protokoll.
- **Einrichtungsassistent (Plattformpfade)**: Der Bibliotheksschritt zeigt nun auch die automatisch erkannten allgemeinen Spieleordner von GOG Galaxy, Ubisoft Connect und der EA App in eigenen schreibgeschützten Pfadfeldern an, statt jeden installierten Spielordner einzeln aufzulisten.
- **Einstellungen (Plattformpfade)**: Die allgemeinen Spieleordner von GOG Galaxy, Ubisoft Connect und der EA App werden nun auch im Bibliotheksbereich der Einstellungen angezeigt.
- **Bibliothek (Klick auf Spielkarte)**: Beim Öffnen des Detailfensters scrollt die Bibliothek die angeklickte Kachel nicht mehr unnötig an den oberen Fensterrand. Sichtbare Karten bleiben beim Anklicken stabil an ihrer Position.
- **Bibliothek (Klick auf Spielkarte)**: Beim Öffnen des Detailfensters scrollt die Bibliothek die angeklickte Kachel nicht mehr unnötig an den oberen Fensterrand. Sichtbare Karten bleiben beim Anklicken stabil an ihrer Position.
- **Bibliothek (Ruckler beim Scrollen und Filterwechsel)**: Bild-Caching, Viewport-Retention und Vorladeverhalten wurden so angepasst, dass Filterwechsel, schnelles Scrollen und das Zurückkehren in größere Listen deutlich weniger Nachladeruckler verursachen.
- **App-Start (Wahrgenommene Trägheit)**: Scanner-, Metadaten- und Cover-Ladepfade wurden entkoppelt und gestrafft, wodurch der Launcher beim Start deutlich früher einen vollständigen und stabilen Bibliothekszustand erreicht.

## [2.0.1] - 2026-06-21
### Behoben
- **Update-Erkennung (Release-Assets)**: Die Update-Prüfung erkennt nun auch versionierte Installer-Dateinamen wie `GameLauncher_Setup_2.0.0.0.exe`, sodass GitHub-Releases mit Versionssuffix wieder korrekt gefunden werden.
- **Bibliothek (manuelle Spiele löschen)**: Das virtualisierte Wrap-Panel wurde gegen inkonsistente WPF-Containerzustände beim Entfernen von Einträgen gehärtet. Manuell hinzugefügte Spiele lassen sich nun ohne `NullReferenceException` löschen, und entfernte Kacheln bleiben nicht mehr bis zum nächsten Reload sichtbar.
- **Bibliothek (Suche leeren / Scrollen)**: Das Panel räumt realisierte Kindelemente bei Listenänderungen nun gezielter nur noch im problematischen Remove-Fall auf. Dadurch werden unnötige komplette Neuaufbauten nach dem Leeren der Suche vermieden und kurzes Nachruckeln beim anschließenden Scrollen reduziert.

### Entfernt
- **Legacy-Artwork-Migration**: Der verbliebene Kompatibilitätscode für den alten `Documents/GameLauncher/Cache`-Ordner wurde entfernt. Der Launcher migriert keine alten Artwork-Dateien oder Bildpfade mehr automatisch und berücksichtigt den Legacy-Ordner auch nicht mehr bei Bild-Bereinigungen.
...
### Geändert
- **Open Source**: Das Projekt ist ab Version 2.0 als öffentliches Open-Source-Repository auf GitHub verfügbar.
